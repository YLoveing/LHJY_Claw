...
def load_hs300_stocks():
    ...
    for rec in stock_records:
        ...
        try:
            inc_ymd = pd.Timestamp(inc).strftime("%Y%m%d")
        except:
            inc_ymd = "20000101"
    ...
    for cd in ["20240515", "20240930", "20250212", "20251216", "20260522"]:
        n = sum(
            1
            for code, inc in include_dates.items()
            if cd >= inc and len(all_kline[code][all_kline[code]["date"] <= pd.Timestamp(cd)]) >= 30
        )
...
def run_backtest(all_kline, hs300, include_dates, use_market_filter):
    all_dates = sorted(set(d for df in all_kline.values() for d in df["date"].dt.strftime("%Y%m%d").values))
...
def main():
    ...
    print("\n" + "=" * 100)
    print(f"{'策略':<30} {'收益%':>8} {'最终':>10} {'回撤%':>8} " f"{'夏普':>8} {'胜率%':>8} {'均仓':>6} {'排除':>6}")
    print("=" * 100)
    for r in results:
        print(
            f"{r['strategy']:<30} {r['total_return_pct']:>+8.2f} "
    ...
    if v3_reports:
        ...
        print(f"\n与旧版(V3, 无幸存者修正)对比:")
        print(
            f"  V1纯技术: 旧{old_v1['total_return_pct']:+.2f}% → 新{r1['total_return_pct']:+.2f}%"
            f"  (Δ{r1['total_return_pct']-old_v1['total_return_pct']:+.2f}%)"
        )
        print(f"  回撤:     旧{old_v1['max_drawdown_pct']:.2f}% → 新{r1['max_drawdown_pct']:.2f}%")
        print(
            f"  V3均线过滤: 旧{old_v3['total_return_pct']:+.2f}% → 新{r2['total_return_pct']:+.2f}%"
    ...
    with open(out, "w") as f:
        json.dump(
            {"run_date": datetime.now().isoformat(), "stock_count": len(all_kline), "strategies": results},
            f,
            ensure_ascii=False,
            indent=2,
        )
