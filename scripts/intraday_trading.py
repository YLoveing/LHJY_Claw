...
from simulated_trading import MAX_POSITIONS as SHARED_MAX_POS
from simulated_trading import (
    calc_buy_fees,
    calc_sell_fees,
...
def record_signal(
    code: str, name: str, signal: str, detail: str, price: float, volume: int, vol_ratio: Optional[float]
) -> Optional[Dict]:
    """
    记录建仓信号到待执行队列（不立即买入）。
    ...
    log.info(f"  📝 [信号记录] {name}({code}) 信号:{signal} " f"价¥{price:.3f} 权重:{weight:.1f} 等待次日执行")
...
def execute_pending_signals() -> List[Dict]:
    ...
    for s in pending_active:
        ...
        log.info(
            f"  ✅ [信号执行] 买入 {name}({code}) × {quantity} @ ¥{price:.3f} = ¥{total_cost:.0f} "
...
def check_positions() -> List[Dict]:
    ...
    intraday_codes = [code for code, pos in state["positions"].items() if pos.get("mode") == "intraday"]
    ...
    for code in list(intraday_codes):
        ...
        # 条件③：跟踪止损（浮盈达标后，从最高点回撤）
            ...
            if drawdown_from_peak <= TRAILING_STOP_PCT:
                ...
                reason = (
                    f"跟踪止损：最高¥{pos['highest_price']:.3f} 回撤{drawdown_from_peak:.1f}%（≤{TRAILING_STOP_PCT}%）"
                )
        ...
        if action:
            ...
            trade = {
                ...
                "reason": reason,
                "hold_days": (
                    (now - datetime.strptime(pos.get("entry_date", today), "%Y%m%d")).days
                    if pos.get("entry_date") and len(pos["entry_date"]) == 8
                    else 0
                ),
            }
            ...
            log.info(
                f"  {'🔴' if action in ('stop_loss','trailing_stop') else '🟢'} [日内交易] "
...
def get_status_text() -> str:
    ...
    intraday_positions = {c: p for c, p in state.get("positions", {}).items() if p.get("mode") == "intraday"}
    daily_positions = {c: p for c, p in state.get("positions", {}).items() if p.get("mode") != "intraday"}
    ...
    if intraday_positions:
        ...
        for code, pos in intraday_positions.items():
            lines.append(
                f"  {pos.get('name',code)}({code}) {pos['quantity']}股 "
