        ...
        try:
            ...
            if cached is not None and not cached.empty:
                ...
                if "date" in cached.columns:
                    ...
                    req_start = pd.Timestamp(start_date) if start_date else cache_min
                    req_end = pd.Timestamp(end_date) if end_date else pd.Timestamp.now()
                    if cache_min <= req_start and cache_max >= req_end:
                        logger.info(f"[磁盘缓存命中] {stock_code}: {len(cached)}行 " f"({cache_min} ~ {cache_max})")
                        return cached, "disk_cache"
                    else:
                        logger.info(
                            f"[磁盘缓存不完整] {stock_code}: 缓存 {cache_min}~{cache_max}, "
